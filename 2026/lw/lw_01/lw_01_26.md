...
11. Повторить ключевые описательные расчёты в BigQuery и сопоставить их с результатами `pandas`.
...
- выполнять базовые агрегаты в BigQuery и сопоставлять их с результатами `pandas`.
...
flowchart TD
    ...
    F --> G[Добавление<br/>медиана или среднее]
...
## 7. Пример постановки задачи для варианта № 25

Для учащихся с `lunch == "standard"` сравните `writing score` в группах `test preparation course == "completed"` и `none`.

1. Проверьте качество данных и выполните очистку варианта: числовые пропуски — медианой, категориальные — модой, потенциальные выбросы — методом IQR.
2. Рассчитайте описательные статистики и визуализируйте распределения.
3. Проверьте гипотезу односторонним Welch t-test при $\alpha=0.05$:
...
4. Сформулируйте **короткий вывод**: различаются ли группы и почему полученный результат нельзя трактовать как доказательство причинного влияния курса.
...
## Дополнительное требование: расчёты в BigQuery

После выполнения основной части в Colab повторите ключевые **описательные расчёты** варианта в BigQuery.

### 1. Подготовьте файл для загрузки

В Colab сохраните очищенный `df_clean`, предварительно заменив названия столбцов на удобные для SQL:

```python
bq_export = df_clean.rename(columns={
...
bq_export.to_csv("students_performance_lab01_clean.csv", index=False)
```

Для варианта № 25 такой файл должен содержать **988 строк**.

### 2. Загрузите CSV в BigQuery
...
### 3. Выполните расчёты своего варианта

Минимально требуется:

1. проверить число строк через `COUNT(*)`;
2. повторить фильтр варианта с `WHERE`;
3. получить групповые показатели с `COUNT`, `AVG`, `MIN`, `MAX` и `GROUP BY` — в зависимости от варианта;
4. если вариант связан с корреляцией, использовать `CORR()`;
5. сравнить результаты BigQuery и Colab.

Для варианта № 25 пример:

```sql
SELECT
  test_preparation,
  COUNT(*) AS n,
  ROUND(AVG(math_score), 2) AS mean_math,
  ROUND(AVG(reading_score), 2) AS mean_reading,
  ROUND(AVG(writing_score), 2) AS mean_writing
FROM `YOUR_PROJECT_ID.learning_analytics.students_performance`
WHERE lunch = 'standard'
GROUP BY test_preparation
ORDER BY test_preparation;
```

Ожидаемый результат для эталонного решения:

| test_preparation | n | mean_math | mean_reading | mean_writing |
|---|---:|---:|---:|---:|
| `completed` | 227 | 73.53 | 76.22 | 76.77 |
| `none` | 416 | 68.34 | 69.36 | 67.79 |

Короткое пояснение: BigQuery должен воспроизвести те же описательные показатели, которые были получены в `pandas`. Если результаты отличаются, сначала проверьте, что в обеих средах используются одинаковые очищенные данные и одинаковый фильтр.

> Welch t-test в этой лабораторной выполняется в Colab. В BigQuery требуется повторить именно базовые описательные и групповые расчёты.

### Что приложить к отчёту

- текст SQL-запроса;
- результат запроса или один скриншот BigQuery;
- 1–2 предложения о совпадении результатов с Colab.
...
| Критерий | Баллы |
|---|---:|
| Загрузка, проверка и очистка данных в Colab | **2** |
| Индивидуальный анализ и описательная статистика | **2** |
| Визуализации | **1** |
| Проверка гипотезы и корректный статистический вывод | **2** |
| Дополнительные расчёты в BigQuery и сопоставление с Colab | **2** |
| Методическая разработка и оформление работы | **1** |
| **Итого** | **10** |
...
8. SQL-запрос(ы) BigQuery и результат/скриншот выполнения;
9. короткое сравнение результатов Colab и BigQuery.
