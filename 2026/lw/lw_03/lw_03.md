...
## NLP-анализ англоязычного корпуса и классификация новостных текстов
...
В `scikit-learn` реализация включает детали нормировки и сглаживания; поэтому ручная формула и значения `TfidfVectorizer` могут не совпадать буквально.
...
Macro-$F_1$ одинаково учитывает каждую категорию независимо от её размера.
